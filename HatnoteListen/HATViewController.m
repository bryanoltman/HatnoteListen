...
int indexForChangeSize(double changeSize) {
  ...
  //  var pitch = 100 - Math.min(max_pitch, Math.log(size + log_used) /
  //  Math.log(log_used)); var index = Math.floor(pitch / 100.0 *
  //  Object.keys(celesta).length); var fuzz = Math.floor(Math.random() * 4) -
  //  2; index += fuzz; index = Math.min(Object.keys(celesta).length - 1,
  //  index); index = Math.max(1, index); if (current_notes < note_overlap) {
  //      current_notes++;
  //      if (type == 'add') {
  //          celesta[index].play();
  //      } else {
...
@interface HATViewController () <HATUserJoinedBannerDelegate,
                                 HATWikipediaServiceDelegate,
                                 HATArticleTitleViewDelegate>
...
- (void)animateSelectionChangeFromView:(HATUpdateView*)fromView toView:(HATUpdateView*)toView {
  ...
  if (toView && toView.superview) {
    ...
    CGPoint toPoint = CGPointMake(self.view.center.x, CGRectGetHeight(self.view.frame) -
                                                          self.view.safeAreaInsets.bottom - 34 -
...
- (void)userJoinedBannerTapped:(HATUserJoinedBanner*)banner {
  NSString* urlString =
      [NSString stringWithFormat:@"http://%@.wikipedia.org/w/"
                                 @"index.php?title=User_talk:%@&action=edit&section=new",
...
- (void)articleTitleViewTapped:(HATArticleTitleView*)articleTitleView {
  ...
  [[UIApplication sharedApplication] openURL:articleTitleView.articleURL
                                     options:@{}
                           completionHandler:nil];
}
